...
from pyspark.sql.types import IntegerType, DoubleType, StructType, StructField, StringType, ArrayType, Row
...
def explode_hobbies():
    data = [("John", ["football", "swiming", "reading"]), ("Alice", ["danse", "reading"]),
...
def explode_items():
    ...
    df = df.select(*df.columns, F.count(F.col('*')).over(window).alias('num_orders'))
    df.show()
